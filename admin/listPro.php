...
//得到数据库中所有商品,分页显示
$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from product  p join cate  c on p.cId=c.id";
connect();
...
$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from product  p join cate  c on p.cId=c.id limit {$offset},{$pageSize}";
$rows=fetchAll($sql);
	            ...
	            <td align="center">
	                ...
	                <table class="table" cellspacing="0" cellpadding="0">
	            		...
	            		<tr>
	            			...
	            			?>
	            			<img width="160" height="90" src="./uploads/<?php echo $img['albumPath'];?>" alt=""/> &nbsp;&nbsp;
	            			<?php endforeach;?>
	            			</td>
	            		</tr>
	            		<tr>
